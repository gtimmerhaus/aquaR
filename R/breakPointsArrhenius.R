...
breakPointsArrhenius <- function(x, group, col="black", marks=T, obs=3, error.bars=T,
    ...
    for(i in 1:nrow(temp)){
        bestCor <- 1
        lowestP <- 1
        for(ii in (ncol(temp)-2):2){ #find the best correlation 
        ...
        model.x <- c((res[i,2]-1):(res[i,2]-(obs+1))) #x-values for the following linear model 
        beyInc <- lm(as.numeric(temp[i,(res[i,2]-1):(res[i,2]-(obs+1))])~model.x[model.x!=0]) #lm() for beyond breakpoint observations (n=obs), if there are 0s in points.x, there are removed
        ...
        points(c(xpos, floor(xpos)-obs), c(ypos, beyInc$coefficients[1]+beyInc$coefficients[2]*(floor(xpos)-obs)), 
               col=col[i], type="l")
        #draw indicator line:
