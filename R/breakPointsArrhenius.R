...
breakPointsArrhenius <- function(x, group, col="black", marks=T, obs=3, error.bars=T,
  ...
  temp <- m[,-1] #all columns minus the group column
  for(i in 1:nrow(temp)){
    bestCor <- 1
    lowestP <- 1
    for(ii in (ncol(temp)-2):2){ #find the best correlation or lowest p-value (depending on method = "correl" | "pvalue")
      if(cor(as.numeric(m[i,ii:ncol(temp)]),ii:ncol(temp), use="complete.obs") < bestCor){#find the best correlation from last point to ii
    ...
    #add linear models for best correlation (by adding point after point):
    linInc <- lm(as.numeric(temp[i,ncol(temp):res[i,2]])~c(ncol(temp):res[i,2])) #lm() for linear increase
    beyInc <- lm(as.numeric(temp[i,(res[i,2]-1):(res[i,2]-(obs+1))])~c((res[i,2]-1):(res[i,2]-(obs+1)))) #lm() for beyond breakpoint observations (n=obs)
    ...
    points(c(xpos, floor(xpos)-obs), c(ypos, beyInc$coefficients[1]+beyInc$coefficients[2]*(floor(xpos)-obs)), col=col[i], type="l")
    #draw indicator line:
